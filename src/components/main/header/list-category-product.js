	...
	render() {
		...
		if (this.props.dataCategoryGroup.length > 0) {
			this.props.dataCategoryGroup.forEach(e => {
				let lisItemCategory = [];
				if (e.categories ? e.categories.length > 0 : false) {
					e.categories.forEach(i => {
						lisItemCategory.push(<li key={ i.id }>
				...
				listItemCategoryGroup.push(<li key={ e.id }>
                                 <Link to={ `/${e.urlKey}` }>
...
ListCategoryProduct.propTypes = {
	dataCategoryGroup: React.PropTypes.array
};
